        ...
        foreach (var layer in layers)
        {
            ...
            if (layer.HasImage && createTexture)
            {
                ...
                if (replaceByName)
                {
                    ...
                    {
                        var assetPath = UnityEditor.AssetDatabase.GUIDToAssetPath(findedSprites[0]);
                        var objs2 = UnityEditor.AssetDatabase.LoadAllAssetsAtPath(assetPath);
                        if (objs2.Length > 0) {
                            foreach (var o in objs2)
                            {
                                if (o is Sprite && o.name.ToLower().Contains(layer.Name.ToLower()))
                                {
                                    sprite = o as Sprite;
                                }
                            }
                        } else {
                            sprite = (objs2[0]) as Sprite;
                        }
                    }
                }
#endif
